 ...
 * @LastEditTime: 2024-11-08 15:51:17
...
- **其他功能：**
  ...
  - 支持`语法高亮`
  - 资源管理器中右键目录，新增两个菜单: `创建小程序页面` , `创建小程序组件` (默认`关闭`，可在插件 `设置` 中开启) 
  - 支持在 `wxml` 页面，`alt + 点击自定义组件的标签名` 跳转到对应的组件页面 (默认`关闭`，可在插件 `设置` 中开启) 
  - 支持`组件高亮` (默认`关闭`，可在插件 `设置` 中开启) 
